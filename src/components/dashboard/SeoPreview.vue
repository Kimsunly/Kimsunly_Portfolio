...
const scoreColor = computed(() => seoScore.value >= 80 ? '#22c55e' : seoScore.value >= 50 ? '#f59e0b' : '#ef4444')
const scoreLabel = computed(() => seoScore.value >= 80 ? 'Excellent' : seoScore.value >= 50 ? 'Needs Work' : 'Poor')
  ...
  <div class="flex flex-col gap-5 w-full">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      ...
      <button @click="copyMeta"
        class="flex items-center gap-2 px-4 py-2 rounded-lg border border-border bg-surface text-xs font-mono text-text-muted hover:text-text-primary hover:border-primary transition-all duration-150">
        <Copy :size="13" />
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="bg-[#f0f2f5] rounded-xl overflow-hidden border border-gray-200">
            <div class="h-16 bg-gradient-to-r from-violet-600 to-indigo-700 flex items-center justify-center">
              <span class="text-white font-bold text-base tracking-wide truncate px-4">
    ...
    <!-- SEO Score — full-width card below both columns -->
    <div class="bg-surface border border-border rounded-xl p-4 flex items-center gap-5">
      <!-- Ring gauge -->
      <div class="relative flex-shrink-0 w-16 h-16">
        <svg viewBox="0 0 52 52" width="64" height="64" class="-rotate-90">
          <circle cx="26" cy="26" r="22" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="5" />
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 mb-2">
          ...
          <span class="font-mono text-xs px-2 py-0.5 rounded-full border font-semibold"
            :style="{ color: scoreColor, borderColor: scoreColor + '60', background: scoreColor + '18' }">
            {{ scoreLabel }}
          </span>
        </div>
